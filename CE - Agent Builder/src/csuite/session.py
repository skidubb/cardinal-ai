...
class DebateArgument(BaseModel):
    """A single agent's argument in one debate round."""
...
class DebateSession(BaseModel):
    """A full multi-round debate session."""
    ...
    def get_all_arguments_through_round(self, n: int) -> list[DebateArgument]:
        """Return all arguments from rounds 1..n."""
        args: list[DebateArgument] = []
        for rnd in self.rounds:
            if rnd.round_number <= n:
                args.extend(rnd.arguments)
        return args

    def add_round(self, debate_round: DebateRound) -> None:
        """Add a completed round to the debate."""
        self.rounds.append(debate_round)
        self.updated_at = datetime.now()

    def set_synthesis(self, synthesis: str) -> None:
        """Set the final synthesis and mark debate complete."""
...
class DebateSessionManager:
    """Manages debate session persistence via DuckDB."""

    def __init__(self, session_dir: Any = None):
        # session_dir kept for signature compat but unused with DuckDB
        pass

    def save(self, debate: DebateSession) -> None:
        db = _get_store()
        db.save_debate(debate.model_dump(mode="json"))

    def load(self, debate_id: str) -> DebateSession | None:
        db = _get_store()
        data = db.load_debate(debate_id)
        if data is None:
            return None
    ...
    def list_sessions(self, limit: int = 20) -> list[DebateSession]:
        db = _get_store()
        rows = db.list_debates(limit=limit)
        return [DebateSession(**row) for row in rows]

    def delete(self, debate_id: str) -> bool:
        db = _get_store()
        return db.delete_debate(debate_id)
