...
<task id="4">
Verify `protocols/llm.py` line 171 (`from protocols.cost_tracker import _compute_cost`) still works by running: `cd "CE - Multi-Agent Orchestration" && python -c "from protocols.cost_tracker import _compute_cost; print(_compute_cost('claude-opus-4-7', 1000, 1000))"`. If the import fails, fix it.
</task>
<task id="5">
...
<task id="7">
Run a quick smoke test: `cd "CE - Multi-Agent Orchestration" && python -c "from protocols.cost_tracker import ProtocolCostTracker; t = ProtocolCostTracker(); t.track('claude-opus-4-7', 1000, 500, 0); print(t.summary())"`. Verify the cost is non-zero and uses correct pricing.
</task>
</tasks>
...
3. `python -c "from protocols.cost_tracker import _compute_cost; print(_compute_cost('claude-opus-4-7', 1000, 1000))"` succeeds from the Orchestration directory
