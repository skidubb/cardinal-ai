...
- `ModelTier(StrEnum)` with OPUS, SONNET, HAIKU (values are the canonical model ID strings: `claude-opus-4-7`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`)
...
2. `cost_for_model("claude-opus-4-7", 1_000_000, 1_000_000)` returns expected value
...
5. Substring fallback: `cost_for_model("anthropic/claude-opus-4-7", 1000, 1000)` resolves to Opus
