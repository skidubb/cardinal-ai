"""CLI entry point for P51: Wildcard Walk Protocol."""
...
from .orchestrator import WildcardWalkOrchestrator
...
def main():
    parser = argparse.ArgumentParser(description="P51: Wildcard Walk Protocol")
    ...
    print(f"Running Wildcard Walk with {len(agents)} agents")

    if args.dry_run:
        print("[dry-run] Wildcard Walk, no LLM calls.")
        return

    orchestrator = WildcardWalkOrchestrator(
        agents=agents,
    ...
    try:
        ...
        asyncio.run(persist_run(
            protocol_key="p51_wildcard_walk",
