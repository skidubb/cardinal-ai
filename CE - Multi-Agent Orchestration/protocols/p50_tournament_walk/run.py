"""CLI entry point for P50: Tournament Walk Protocol."""
...
from .orchestrator import TournamentWalkOrchestrator
...
def main():
    parser = argparse.ArgumentParser(description="P50: Tournament Walk Protocol")
    ...
    print(f"Running Tournament Walk with {len(agents)} agents")

    if args.dry_run:
        print("[dry-run] Tournament Walk, no LLM calls.")
        return

    orchestrator = TournamentWalkOrchestrator(
        agents=agents,
    ...
    try:
        ...
        asyncio.run(persist_run(
            protocol_key="p50_tournament_walk",
