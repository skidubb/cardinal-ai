      ...
      {present.map((scope) => (
        <section key={scope} className="space-y-2">
          <h3 className="text-xs uppercase tracking-wider text-slate-400">
            {scope}{" "}
            <span className="text-slate-600 ml-1">({byScope[scope].length})</span>
          </h3>
          <ul className="space-y-2">
            {byScope[scope].map((c) => (
              <li
                key={c.id}
                className="rounded-lg border border-slate-800 bg-slate-900/40 p-3 flex items-start justify-between gap-3"
              >
                ...
                <button
                  ...
                  className="text-[10px] uppercase tracking-wider px-2 py-1 rounded border border-slate-700 text-slate-400 hover:text-rose-300 hover:border-rose-700 transition disabled:opacity-40"
                >
